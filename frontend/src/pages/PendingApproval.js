...
const PendingApproval = () => {
  ...
  // كل 30 ثانية يتحقق لو الأدمن وافق
  useEffect(() => {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      if (user?.is_approved) {
        ...
        alert('لم يتم الموافقة بعد، حاول مرة أخرى لاحقاً');
      }
    } catch {
      ...
      <div style={s.card}>
        ...
        <div style={s.infoBox}>
          ...
          <ul style={s.infoList}>
            ...
            <li>ستصلك رسالة بريد إلكتروني عند الموافقة</li>
            <li>بعد الموافقة ستتمكن من نشر الكورسات والتجارب</li>
          </ul>
        </div>

        <div style={s.btnRow}>
          <button style={s.checkBtn} onClick={handleCheck} disabled={checking}>
...
const s = {
  ...
  checkBtn: { flex:1, padding:'12px', background:'linear-gradient(135deg,#c8973a,#a07020)', color:'#000', border:'none', borderRadius:10, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit' },
